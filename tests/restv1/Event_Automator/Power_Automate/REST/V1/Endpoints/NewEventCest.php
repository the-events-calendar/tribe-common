...
class NewEventCest extends BaseRestPowerAutomateCest {

	/**
	 * @inheritdoc
	 */
	protected static $current_test_url = 'new-events';

	/**
	 * @test
	 */
	public function it_should_return_error_when_missing_token_parameters( Restv1Tester $I ) {
		$I->sendGET( $this->new_events_url );
	...
	public function it_should_return_invalid_when_access_token_contains_unverified_consumer_id_and_secret( Restv1Tester $I ) {
		$I->sendGET( $this->new_events_url, [ 'access_token' => static::$invalid_access_token ] );
		$I->seeResponseCodeIs( 401 );
		$I->seeResponseIsJson();
		$response = json_decode( $I->grabResponse(), true );
	...
	public function it_should_return_valid_when_access_token_contains_verified_consumer_id_and_secret_but_no_event_msg( Restv1Tester $I ) {
		...
		$connections = $I->grabOptionFromDatabase( 'tec_power_automate_connections' );
		$connection = $I->grabOptionFromDatabase( 'tec_power_automate_connection_4689db48b24f0ac42f3f0d8fe027b8f28f63f262b9fc2f73736dfa91b4045425' );
		$testurl = $this->new_events_url;

		$I->sendGET( $this->new_events_url, [ 'access_token' => static::$valid_access_token ] );
	...
	public function it_should_return_valid_with_access_token_contains_verified_api_key_pair_and_last_access_is_updated( Restv1Tester $I ) {
		...
		$endpoint_details = get_option( '_tec_power_automate_endpoint_details_new_events' );
	...
	public function it_should_return_valid_with_access_token_but_with_invalid_event_id( Restv1Tester $I ) {
		$invalid_id = [ 'post_id' ];
		$this->setup_new_event_queue( $I, $invalid_id );
		$this->setup_api_key_pair( $I );
		$I->sendGET( $this->new_events_url, [ 'access_token' => static::$valid_access_token ] );
		$I->seeResponseCodeIs( 200 );
		$I->seeResponseIsJson();
	...
	public function it_should_return_valid_with_access_token_but_with_invalid_post_type( Restv1Tester $I ) {
		...
		$this->setup_new_event_queue( $I, $invalid_post_type );
	...
	public function it_should_process_event_queue( Restv1Tester $I ) {
		...
		$this->setup_new_event_queue( $I, $event_ids );
		$this->setup_api_key_pair( $I );

		$event_ids[] = 'no-new-events';
		foreach ( $event_ids as $event_id ) {
			$I->sendGET( $this->new_events_url, [ 'access_token' => static::$valid_access_token ] );
			$I->seeResponseCodeIs( 200 );
			$I->seeResponseIsJson();
			$response = json_decode( $I->grabResponse(), true );
			if ( $event_id === 'no-new-events' ) {
				$I->assertArrayHasKey( 'id', $response['events'] );
				$I->assertEquals( $event_id, $response['events']['id'] );
			} else {
				$I->assertArrayHasKey( 'id', $response['events'][0] );
				$I->assertEquals( $event_id, $response['events'][0]['id'] );
			}
		}
	}
	...
	public function it_should_return_404_when_endpoint_disabled( Restv1Tester $I ) {
		...
		$this->disable_endpoint( $I, '_tec_power_automate_endpoint_details_new_events', $endpoint );
		$this->setup_api_key_pair( $I );
		$I->sendGET( $this->new_events_url, [ 'access_token' => static::$valid_access_token ] );
