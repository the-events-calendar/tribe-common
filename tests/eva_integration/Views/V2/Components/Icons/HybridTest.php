...
class HybridTest extends Snapshot_Test_Case {
	use SnapshotAssertions;

	/**
	 * @var string The path to the template, either relative to the `/src` directory, or absolute.
	 */
	protected $template_path = 'views/v2/components/icons/hybrid.php';
	...
	public function test_render_with_label_no_classes() {
		$this->assertMatchesHtmlSnapshot( $this->render( [ 'icon_label' => 'Hybrid Event' ] ) );
	}
	...
	public function test_render_with_classes_and_label() {
		$this->assertMatchesHtmlSnapshot( $this->render( [ 'classes' => [ 'test-class-1', 'test-class-2' ], 'icon_label' => 'Hybrid Event' ] ) );
	}
}
