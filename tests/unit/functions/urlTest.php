<?php
/**
 * Original tests from jakeasmith/http_build_url
 * @see https://raw.githubusercontent.com/jakeasmith/http_build_url/master/tests/HttpBuildUrlTest.php
 */
...
class urlTest extends \Codeception\Test\Unit {
	...
	public function testTrailingSlash( $expected, $config ) {
		$this->assertEquals( $expected, tribe_build_url( $config ) );
	}
	...
	public function testPartsQueryArrayIsIgnored() {
		...
		$actual   = tribe_build_url( $this->full_url, array( 'query' => array( 'foo' => 'bar' ) ) );
	...
	public function testAcceptStrings() {
		...
		$actual   = tribe_build_url( $this->full_url, 'http://foobar.com:8080' );
	...
	public function testAcceptArrays() {
		$expected = 'http://user:pass@foobar.com:8080/pub/index.php?a=b#files';
		$actual   = tribe_build_url( parse_url( $this->full_url ), parse_url( 'http://foobar.com:8080' ) );
	...
	public function testDefaults() {
		$expected = $this->full_url;
		$actual   = tribe_build_url( $this->full_url );
	...
	public function testNewUrl() {
		...
		tribe_build_url( $this->full_url, null, null, $actual );
	...
	public function testJoinQuery( $query, $expected ) {
		$actual = tribe_build_url( $this->full_url, array( 'query' => $query ), HTTP_URL_JOIN_QUERY );
	...
	public function testJoinPath( $path, $expected ) {
		$actual = tribe_build_url( $this->full_url, array( 'path' => $path ), HTTP_URL_JOIN_PATH );
	...
	public function testBitmasks( $constant, $expected ) {
		$actual = tribe_build_url( $this->full_url, array(), constant( $constant ) );
