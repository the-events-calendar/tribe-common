...
( function ( global ) {
	...
	A11yDialog.prototype.render = function ( event ) {
		const contentNode = getNodes( this.trigger[ 0 ].dataset.content )[ 0 ];
		if ( ! contentNode ) {
			return this;
		}
		const ariaDescribedBy = this.options.ariaDescribedBy
			? 'aria-describedby="' + this.options.ariaDescribedBy + '" '
			: '';
		const ariaLabel = this.options.ariaLabel ? 'aria-label="' + this.options.ariaLabel + '"' : '';
		const ariaLabelledBy = this.options.ariaLabelledBy
		...
		node.innerHTML =
			'<div data-js="a11y-overlay" tabindex="-1" class="' +
			this.options.overlayClasses +
			'"></div>\n' +
			'  <div class="' +
			this.options.contentClasses +
			...
			'           class="' +
			this.options.closeButtonClasses +
			'" ' +
			'           type="button" ' +
			'           aria-label="' +
			this.options.closeButtonAriaLabel +
			'"' +
			'       ></button>\n' +
			'      ' +
