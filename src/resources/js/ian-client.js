( function ( Ian ) {
	window.addEventListener( 'load', function ( event ) {
		...
		const getParentPosition = () => {
			...
			if ( settingstabs ) {
				settingstabs =
					window.innerWidth > 500 ? settingstabs : document.querySelector( '.tec-settings-header-wrap' );
		...
		const whichPlugin = () =>
			document.body.classList.contains( 'tickets_page_tec-tickets-settings' ) ? 'et' : 'tec';
