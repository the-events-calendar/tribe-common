 ...
 * @return {string} return a moment.js valid format
 */
 ...
 * @param {boolean}              parseFormat the parse of the format default to true
 * @return {moment} A moment object
 */
 ...
 * @return {moment} moment Object with the date or current date if is non valid
 */
 ...
 * @return {moment} A moment object where the date is replaced
 */
 ...
 * @return {moment} A moment object with the new date
 */
 ...
 * @return {number} Total of seconds from start of the day to the current moment,
 */
 ...
 * @return {string} A date time format
 */
export const toDateTime = ( date, format = dateUtil.FORMATS.DATABASE.datetime ) => date.format( toFormat( format ) );

export const toDate = ( date, format = dateUtil.FORMATS.WP.date ) => date.format( toFormat( format ) );

export const toDateNoYear = ( date, format = dateUtil.FORMATS.WP.dateNoYear ) => date.format( toFormat( format ) );

export const toTime = ( date, format = dateUtil.FORMATS.WP.time ) => date.format( toFormat( format ) );

export const toTime24Hr = ( date, format = dateUtil.FORMATS.WP.time24Hr ) => date.format( toFormat( format ) );

export const toDatabaseDate = ( date, format = dateUtil.FORMATS.DATABASE.date ) => date.format( toFormat( format ) );

export const toDatabaseTime = ( date, format = dateUtil.FORMATS.DATABASE.time ) => date.format( toFormat( format ) );

export const toDatePicker = ( date = moment(), format = 'YYYY-MM-DDTHH:mm:ss' ) => date.format( format );
 ...
 * @return {boolean} if the start and end dates are the same day
 */
 ...
 * @return {boolean} true if start and end are on the same month
 */
 ...
 * @param {moment}          start The start date
 * @param {(moment|string)} end   The end date
 * @return {boolean} if the start and end dates have the same year
 */
export const isSameYear = ( start, end ) => toMoment( start ).isSame( toMoment( end ), 'year' );
 ...
 *
 * @param {moment} start The start date
 * @return {{start: {moment}, end: {moment}}} Object with two keys: start, end
 */
