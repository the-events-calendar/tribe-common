		...
		return $this->validator ?? function ( $value ): bool {
			if ( ! is_string( $value ) ) {
				throw InvalidRestArgumentException::create(
					...
					$this->get_name(),
					'tec_rest_invalid_hex_color_parameter',
					// translators: 1) is the name of the parameter.
			...
			if ( ! ( preg_match( '/^#([0-9a-fA-F]{3})$/', (string) $value ) || preg_match( '/^#([0-9a-fA-F]{6})$/', (string) $value ) ) ) {
				throw InvalidRestArgumentException::create(
					// translators: 1) is the name of the parameter.
					sprintf( __( 'Parameter `{%1$s}` must be a valid hex color.', 'the-events-calendar' ), $this->get_name() ),
					$this->get_name(),
					'tec_rest_invalid_hex_color_parameter',
					// translators: 1) is the name of the parameter.
					sprintf( __( 'The parameter `{%1$s}` is not a valid hex color. It should be a string like `#000000` or `#000`.', 'the-events-calendar' ), $this->get_name() )
				);
			}
