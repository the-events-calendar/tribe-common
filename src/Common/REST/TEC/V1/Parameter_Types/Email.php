		...
		return $this->validator ?? function ( $value ): bool {
			if ( ! is_email( $value ) ) {
				throw InvalidRestArgumentException::create(
					...
					sprintf( __( 'Parameter `{%1$s}` must be a valid email address.', 'the-events-calendar' ), $this->get_name() ),
					$this->get_name(),
					'tec_rest_invalid_email_parameter',
					// translators: 1) is the name of the parameter.
					sprintf( __( 'The parameter `{%1$s}` is not a valid email address.', 'the-events-calendar' ), $this->get_name() )
				);
			}
