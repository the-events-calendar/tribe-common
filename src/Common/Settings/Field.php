...
/**
 * Helper class that creates fields for use in Settings.
 *
 * @since TBD
 */
class Field {
	...
	/**
	 * Allowed HTML for label kses.
	 *
	 * @since TBD
	 *
	 * @var array
	 */
	public $allowed_label_html = [
		'a'      => [
			'href' => [],
			'title' => [],
		],
	...
	/**
	 * Allowed HTML for tooltip kses.
	 *
	 * @since TBD
	 *
	 * @var array
	 */
	public $allowed_tooltip_html = [
		'a'      => [
			'class' => [],
			'href' => [],
			'title' => [],
	...
	public function __construct( $id, $field, $value = null ) {
		...
		$label               = wp_kses( $args['label'], $this->allowed_label_html );
		$label_attributes    = $args['label_attributes'];
		$tooltip             = wp_kses( $args['tooltip'], $this->allowed_tooltip_html );
	...
	public function do_field() {
		...
		if ( $this->conditional ) {
			...
			if ( $this->display_callback && is_callable( $this->display_callback ) ) {
				...
				// filter the output
				$field = apply_filters( 'tec_field_output_' . $this->type, $field, $this->id, $this );
				echo apply_filters( 'tec_field_output_' . $this->type . '_' . $this->id, $field, $this->id, $this );
