...
use TEC\Common\Event_Automator\Power_Automate\Triggers\Checkin as Trigger_Checkin;
...
class Checkin extends Abstract_REST_Endpoint {
	...
	protected function get_display_name() : string {
		return _x( 'Checkins', 'Display name of the Power Automate endpoint for checkins.', 'event-automator' );
	}
	...
	public function get_documentation() {
		...
		return [
			'post' => [
				...
				'responses'  => [
					'201' => [
						'description' => _x( 'Returns successful checking of the new checkin queue.', 'Description for the Power Automate Checkin REST endpoint on a successful return.', 'event-automator' ),
						'schema'      => [
							'$ref' => '#/definitions/Power_Automate',
						],
					],
					'400' => [
						'description' => _x( 'A required parameter is missing or an input parameter is in the wrong format', 'Description for the Power Automate Checkin REST endpoint missing a required parameter.', 'event-automator' ),
					],
				],
			],
		];
