...
use TEC\Common\Event_Automator\Zapier\Triggers\New_Events as Trigger_New_Events;
use WP_REST_Server;
use WP_REST_Request;
...
class New_Events extends Abstract_REST_Endpoint {
	...
	protected function get_display_name() : string {
		return _x( 'New Events', 'Display name of the Zapier endpoint for new events.', 'event-automator' );
	}
	...
	public function get_documentation() {
		...
		return [
			'post' => [
				...
				'responses'  => [
					'201' => [
						'description' => _x( 'Returns successful checking of the new event queue.', 'Description for the Zapier New Event REST endpoint on a successful return.', 'event-automator' ),
						'schema'      => [
							'$ref' => '#/definitions/Zapier',
						],
					],
					'400' => [
						'description' => _x( 'A required parameter is missing or an input parameter is in the wrong format', 'Description for the Zapier New Event REST endpoint missing a required parameter.', 'event-automator' ),
					],
				],
			],
		];
	...
	public function READ_args() {
		return [
			'access_token' => [
				...
				'type'              => 'string',
				'description'       => _x( 'The access token to authorize Zapier connection.', 'Description for the Zapier New Event REST endpoint required parameter.', 'event-automator' ),
			],
		];
	}
}
