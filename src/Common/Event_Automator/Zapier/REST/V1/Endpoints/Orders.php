...
use TEC\Common\Event_Automator\Zapier\Triggers\Orders as Trigger_Orders;
use WP_REST_Server;
use WP_REST_Request;
...
class Orders extends Abstract_REST_Endpoint {
	...
	protected function get_display_name() : string {
		return _x( 'Orders', 'Display name of the Zapier endpoint for ticket orders.', 'event-automator' );
	}
	...
	public function get_documentation() {
		...
		return [
			'post' => [
				...
				'responses'  => [
					'201' => [
						'description' => _x( 'Returns successful checking of the new event queue.', 'Description for the Zapier Order REST endpoint on a successful return.', 'event-automator' ),
						'schema'      => [
							'$ref' => '#/definitions/Zapier',
						],
					],
					'400' => [
						'description' => _x( 'A required parameter is missing or an input parameter is in the wrong format', 'Description for the Zapier Order REST endpoint missing a required parameter.', 'event-automator' ),
					],
				],
			],
		];
