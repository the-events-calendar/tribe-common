...
class Tribe__Debug_Bar__Panels__Context extends Debug_Bar_Panel {
	...
	public function render() {
		...
		foreach ( $locations as $key => $rw_data ) {
			...
			$html .= '<td><code>' . ( isset( $context[ $key ] ) ? print_r( $context[ $key ], true ) : 'undefined' ) . '</code></td>'; // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- Debug Bar panel renders variables for inspection.
			$html .= '<td>' . ( false !== $orm_arg_key ? '<code>' . print_r( $orm_arg_key, true ) . ' => ' . print_r( $orm_arg_value, true ) . '</code>' : '' ) . '</td>'; // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- Debug Bar panel renders variables for inspection.
