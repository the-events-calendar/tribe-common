...
class Tribe__Admin__Notice__Plugin_Download {
	...
	/**
	 * The path to the plugin file we're showing a notice for.
	 *
	 * @since 4.8.3
	 *
	 * @var string
	...
	public function show_inactive_plugins_alert() {
		...
		$plugin_names_clean_text     = $this->implode_with_grammar( $plugin_name );
		$req_plugin_names_clean_text = $this->implode_with_grammar( $req_plugins );

		$notice_html_content = '<p>'
		/* translators: %1$s: plugin name, %2$s: plugin name, %3$s: plugin name */
		...
		$pue_notice_text =
		/* translators: %1$s: plugin name, %2$s: read more link */
		esc_html__(
		...
		$pue_notice_html = $has_pue_notices ? '<p>' . sprintf( $pue_notice_text, $plugin_names_clean_text, $read_more_link ) . '</p>' : '';

		printf(
			'<div class="error tribe-notice tribe-dependency-error" data-plugin="%1$s">'
			. wp_kses_post( $notice_html_content )
			. wp_kses_post( $pue_notice_html )
			. '</div>',
			esc_attr( sanitize_title( $plugin_data['Name'] ) ),
			wp_kses( $plugin_names_clean_text, $allowed_html ),
			wp_kses( $req_plugin_names_clean_text, $allowed_html )
		);
	}
