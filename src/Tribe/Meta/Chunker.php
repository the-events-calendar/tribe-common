...
class Tribe__Meta__Chunker {
	...
	protected function delete_chunks( $object_id, $meta_key ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE (meta_key = %s OR meta_key = %s) AND post_id = %d", $chunk_meta_key, $meta_key, $object_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Chunked meta storage manages its own postmeta rows; values are prepared.
	}
	...
	public function get_chunks_for( $object_id, $meta_key ) {
		...
		foreach ( $meta_ids as $meta_id ) {
			$meta_values[] = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM {$wpdb->postmeta} WHERE meta_id = %d", $meta_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Chunked meta storage manages its own postmeta rows; values are prepared.
		}
	...
	public function get_checksum_for( $object_id, $meta_key ) {
		...
		$checksum = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key = %s", $object_id, $this->get_checksum_key( $meta_key ) ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Chunked meta storage manages its own postmeta rows; values are prepared.
	...
	protected function get_all_meta( $object_id ) {
		...
		/** @var wpdb $wpdb */
		global $wpdb;
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value FROM {$wpdb->postmeta} WHERE post_id = %d", $object_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Chunked meta storage manages its own postmeta rows; values are prepared.
