...
class Tribe__Repository__Query_Filters {
	...
	protected function and_field_like( $field, $entry ) {
		...
		$variations = [
			$wpdb->prepare( "{$wpdb->posts}.%i LIKE %s ", $field, "{$like}%" ),
			$wpdb->prepare( "{$wpdb->posts}.%i LIKE %s ", $field, "%{$like}%" ),
			$wpdb->prepare( "{$wpdb->posts}.%i LIKE %s ", $field, "%{$like}" ),
		];
	...
	protected function where_field_is( $where, WP_Query $query, $field, $prepare = '%s' ) {
		...
		$where .= $wpdb->prepare( " AND {$wpdb->posts}.%i = {$prepare} ", $field, $this->query_vars[ $field ] ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare -- $prepare is the placeholder format chosen by the caller.
	 ...
	 *
	 * @since 5.1.5
	 * @since 6.12.0 Made $query_slug explicitly nullable.
	 *
