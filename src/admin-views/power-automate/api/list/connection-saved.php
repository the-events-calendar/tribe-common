...
>
	...
	$this->template( 'components/read-only', [
		...
		'screen_reader' => _x( 'The name for the Power Automate API Key.', 'The screen reader text of the label for the Power Automate API Key name.', 'event-automator' ),
	...
	$this->template( 'components/read-only', [
		...
		'label'         => _x( 'User', 'Label for the user of the API Key for Power Automate.', 'event-automator' ),
		'screen_reader' => _x( 'The user for the Power Automate API Key.', 'The screen reader text of the label for the Power Automate API Key user.', 'event-automator' ),
	...
	$this->template( 'components/read-only', [
		...
		'label'         => _x( 'Last Access', 'Label for the last access of the API Key for Power Automate.', 'event-automator' ),
		'screen_reader' => _x( 'The last access for the Power Automate API Key.', 'The screen reader text of the label for the Power Automate API Key last access.', 'event-automator' ),
	...
	<?php if ( ! empty( $connection_data['name'] ) && ! empty( $connection_data['consumer_secret'] ) ) {
		$this->template( 'components/integration/delete-button', [
	...
	<?php if ( ! empty( $connection_data['name'] ) && ! empty( $connection_data['consumer_id'] ) && strpos( $connection_data['consumer_id'], 'ci_' ) === 0 ) {
		$this->template( 'power-automate/api/components/access', [
			'consumer_id' => $consumer_id,
			'api_key'     => $connection_data,
			'url'         => $url,
		] );
